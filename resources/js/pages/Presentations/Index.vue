    ...
    <AppLayout :breadcrumbs="[{ title: 'Ponencias', href: '/presentations' }]">
        ...
        <div class="mx-auto min-h-screen w-full max-w-7xl space-y-6 px-8 py-8">
            ...
            <div class="mt-6 rounded-xl border border-gray-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                            ...
                            >
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    <div v-for="author in presentation.authors?.slice(0, 2)" :key="author.id" class="text-xs">
                                        {{ author.first_name }} {{ author.last_name }}
                                    </div>
                                    <div v-if="presentation.authors?.length > 2" class="text-xs text-gray-400">
                                        +{{ presentation.authors.length - 2 }} más
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    <div v-if="presentation.day">
                                        {{ presentation.day }}
                                        <div class="text-xs text-gray-400">{{ presentation.start_time }} - {{ presentation.end_time }}</div>
                                    </div>
                            ...
                            <tr v-if="!presentations?.data || presentations.data.length === 0">
                ...
                <div class="flex justify-end gap-4 border-t border-gray-100 px-6 py-4 text-sm font-medium dark:border-zinc-800" v-if="presentations.total > 0">
                    <Link
