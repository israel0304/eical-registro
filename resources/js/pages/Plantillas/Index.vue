...
> = {
    badge: {
        title: 'Plantilla del Gafete',
        headTitle: 'Plantilla del Gafete',
        description:
    ...
    certificate: {
        ...
        headTitle: 'Plantillas de Certificados',
        description:
...
const visibleTabs = computed(() =>
    tabs.filter((t) => props.permissions[t.permission]),
);
...
const defaultTab = (): TabKey => {
    const first = tabs.find((t) => props.permissions[t.permission]);
    return first ? first.key : 'certificate';
};
...
const saveTemplate = () => {
    form.post(activeKind.value.basePath, {
        onSuccess: () => {
...
const deleteTemplate = (id: number) => {
    if (confirm('¿Eliminar esta plantilla?')) {
        router.delete(activeKind.value.basePath + '/' + id, {
            preserveScroll: true,
        });
    }
};
...
<template>
    <AppLayout
        :breadcrumbs="[{ title: 'Plantillas', href: '/admin/plantillas' }]"
    >
        <Head :title="activeKind.headTitle" />
                    ...
                    <form @submit.prevent="saveTemplate">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div v-if="activeKind.requiresType">
                                    <label
                                        class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300"
