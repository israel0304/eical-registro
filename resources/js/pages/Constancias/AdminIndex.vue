<script setup lang="ts">
import { Head } from '@inertiajs/vue3';
import AppLayout from '@/layouts/app/AppSidebarLayout.vue';

defineProps<{
    workshops: any[];
}>();
</script>

<template>
    <AppLayout :breadcrumbs="[{ title: 'Constancias', href: '/admin/constancias' }]">
        <Head title="Administrar Constancias" />

        <div class="mx-auto min-h-screen w-full max-w-7xl space-y-6 px-8 py-8">
            <h1 class="mb-8 text-3xl font-normal tracking-tight text-gray-900 dark:text-white">
                Administrar Constancias
            </h1>

            <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-zinc-800">
                        <thead class="border-b bg-white dark:border-zinc-800 dark:bg-zinc-900">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-900 dark:text-gray-200">
                                    Taller
                                </th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-900 dark:text-gray-200">
                                    Instructor
                                </th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-900 dark:text-gray-200">
                                    Fecha
                                </th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold tracking-wider text-gray-900 dark:text-gray-200">
                                    Inscritos
                                </th>
                                <th scope="col" class="relative px-6 py-4">
                                    <span class="sr-only">Acciones</span>
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                            <tr
                                v-for="workshop in workshops"
                                :key="workshop.id"
                                class="transition-colors duration-150 hover:bg-gray-50 dark:hover:bg-zinc-800"
                            >
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ workshop.name }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ workshop.instructor_name }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ workshop.day }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                    {{ workshop.total_inscritos || 0 }}
                                </td>
                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <span class="text-gray-400 text-xs">
                                        Descarga individual desde el panel de taller
                                    </span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </AppLayout>
</template>
