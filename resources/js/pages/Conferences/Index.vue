...
const openEditModal = (conference: any) => {
    ...
    form.start_time = conference.start_time ? conference.start_time.slice(0, 5) : '';
    form.end_time = conference.end_time ? conference.end_time.slice(0, 5) : '';
    form.members = conference.members?.length
        ? conference.members.map((m: any) => ({
              id: m.id,
              name: `${m.first_name} ${m.last_name}`,
              role: m.pivot?.role || 'speaker',
          }))
        : [];
    showModal.value = true;
};
